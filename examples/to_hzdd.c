...
main(int argc, char **argv)
{
    ...
    /* convert to HZDD */
    states->bdd = hzdd_from_mtbdd(states->bdd, states->variables);
    for (i=0; i<next_count; i++) {
        MTBDD domain = sylvan_and(action_variables, next[i]->variables);
        next[i]->bdd = hzdd_from_mtbdd(next[i]->bdd, domain);
    }

    /* dump every transition to .dot */
    for (int i=0; i<next_count; i++) {
        char filename[64];
        sprintf(filename, "trans-%03d-hzdd.dot", i);
        FILE *f = fopen(filename, "w");
        hzdd_fprintdot(f, next[i]->bdd);
        fclose(f);
    }

    INFO("HZDD nodes:\n");
    INFO("Initial states: %zu HZDD nodes\n", hzdd_nodecount(states->bdd));
    for (i=0; i<next_count; i++) {
        INFO("Transition %d: %zu HZDD nodes\n", i, hzdd_nodecount(next[i]->bdd));
    }

    /* compute total node counts */
    all_dd[0] = states->bdd;
    for (int i=0; i<next_count; i++) all_dd[1+i] = next[i]->bdd;
    INFO("All DDs: %'zu nodes\n", hzdd_nodecount_more(all_dd, 1+next_count));
